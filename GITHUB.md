...
База задач `data/zadachi.db` **попадает** на GitHub — так другу проще развернуть с уже введёнными задачами.

После того как вы локально добавили задачи: в GitHub Desktop нажмите **Commit** → **Push origin**.
...
База задач уже в репозитории (data/zadachi.db).
Скопируй .env.example → .env и задай пароли.
```
---
